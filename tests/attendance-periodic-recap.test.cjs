...
test('rekap GTK hanya berasal dari gtk/absensi_guru dan setiap GTK membawa mapel serta jadwalnya sendiri', () => {
  ...
  assert.deepEqual(guru.mapel_list, ['Matematika'])
  assert.equal(guru.jml_jtm, 1)
  assert.equal(guru.jumlah_kehadiran, 1)
  assert.deepEqual(gtkKosong.mapel_list, ['Bahasa Indonesia'])
  assert.equal(gtkKosong.jml_jtm, 1)
...
test('UI merender daftar Siswa dan GTK sebagai tabel terpisah sesuai tab aktif', () => {
  ...
  assert.match(page, /Mata Pelajaran Diampu/)
  assert.match(page, /Jumlah JTM/)
  assert.match(page, /Jumlah Kehadiran/)
  assert.doesNotMatch(page, /Jadwal mengajar guru pada periode ini/)
})
