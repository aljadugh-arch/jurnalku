...
const tenantSettings = read('server/tenant-settings.cjs')
...
test('favicon is a real tenant image and broken student or branding images have fallbacks', () => {
  ...
  assert.match(server, /getTenantSettings\(db, req\.tenantId, 'pwa_icon,logo'\)/)
  assert.match(tenantSettings, /WHERE id=\? AND tenant_id=\?/)
