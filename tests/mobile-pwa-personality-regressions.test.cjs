...
test('regenerate manifest memakai base URL API dengan benar dan manifest memakai URL stabil', () => {
  ...
  assert.doesNotMatch(indexHtml, /rel="manifest"/)
  ...
  assert.match(main, /linkEl\?\.remove\(\)/)
...
test('backend menyimpan status PWA dan menyajikan manifest tenant dengan benar', () => {
  ...
  assert.match(server, /getTenantSettings\(db, req\.tenantId, 'pwa_enabled,pwa_name,pwa_icon,nama_lembaga,logo,primary_color,pwa_bg_color,pwa_theme_color,updated_at'\)/)
  assert.match(server, /s\.pwa_enabled === 0.*status\(204\)/)
