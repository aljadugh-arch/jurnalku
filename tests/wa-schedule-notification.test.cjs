...
test('dua JTM berurutan yang sama hanya mengantrekan satu notif pada jam pertama', () => {
  ...
  insert.run('j1', 't1', 'g1', 'm1', 'r1', 'Senin', '08:00', '08:40')
  insert.run('j2', 't1', 'g1', 'm1', 'r1', 'Senin', '08:40', '09:20')
  ...
  assert.match(db.prepare('SELECT message FROM wa_queue').get().message, /08:00/)
})

 test('tenant lain tidak ikut antrean', () => {
  const db = new Database(':memory:')
  db.exec(`CREATE TABLE settings (tenant_id TEXT PRIMARY KEY, nama_lembaga TEXT, hari_libur TEXT); CREATE TABLE kalender_kbm (tenant_id TEXT, tanggal TEXT, jenis TEXT); CREATE TABLE notif_settings (tenant_id TEXT PRIMARY KEY, notif_jadwal_guru INTEGER, template_jadwal_guru TEXT); CREATE TABLE tahun_ajaran (tenant_id TEXT, aktif INTEGER, tanggal_mulai TEXT, tanggal_selesai TEXT); CREATE TABLE gtk (id TEXT, tenant_id TEXT, nama TEXT, no_hp TEXT, status TEXT); CREATE TABLE mapel (id TEXT, tenant_id TEXT, nama TEXT); CREATE TABLE rombel (id TEXT, tenant_id TEXT, nama TEXT); CREATE TABLE jadwal (id TEXT, tenant_id TEXT, gtk_id TEXT, mapel_id TEXT, rombel_id TEXT, hari TEXT, jam_mulai TEXT, jam_selesai TEXT);`)
  setupWA(db)
  db.prepare('INSERT INTO settings VALUES (?,?,?)').run('t1', 'Satu', '[]')
  db.prepare('INSERT INTO notif_settings VALUES (?,?,?)').run('t1', 1, '')
  db.prepare('INSERT INTO tahun_ajaran VALUES (?,?,?,?)').run('t1', 1, '2026-01-01', '2026-12-31')
  db.prepare('INSERT INTO gtk VALUES (?,?,?,?,?)').run('g1', 't2', 'Lain', '081234567890', 'aktif')
  db.prepare('INSERT INTO mapel VALUES (?,?,?)').run('m1', 't2', 'Mapel',)
  db.prepare('INSERT INTO rombel VALUES (?,?,?)').run('r1', 't2', 'VII')
  db.prepare('INSERT INTO jadwal VALUES (?,?,?,?,?,?,?,?)').run('j1', 't2', 'g1', 'm1', 'r1', 'Senin', '08:00', '09:00')
  assert.equal(queueDueSchedules(db, { tenantId: 't1', date: '2026-08-24', time: '08:00' }).queued, 0)
})
