...
//
// Google Drive diakses lewat REST + service-account JWT (tanpa dependency googleapis).
...
function loadServiceAccount() {
  const p = process.env.GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON_FILE || SA_FALLBACK
  const raw = fs.readFileSync(p, 'utf8')
  const sa = JSON.parse(raw)
  if (!sa.client_email || !sa.private_key) throw new Error('service account JSON tidak valid (client_email/private_key kosong)')
  return sa
}
...
async function getAccessToken(sa) {
  ...
  if (!resp.ok || !data.access_token) throw new Error('gagal ambil access token: ' + (data.error_description || data.error || resp.status))
  ...
  app.get('/api/google-drive/status', kadmin, async (req, res) => {
    try {
      ...
      try {
        ...
        return res.json({ connected: true, email: sa.client_email, folder_id: folderId, folder_ok })
      } catch (e) {
        return res.json({ connected: false, email: sa.client_email, folder_id: folderId, folder_ok: false, error: String(e.message || e) })
      }
    } catch (e) {
