  ...
  for(const [key,section] of Object.entries(SECTIONS)){
   ...
   for(const table of section.tables){
    ...
    for(const source of legacy.data[table]){
     ...
     for(const column of allowed){if(column==='tenant_id')row[column]=tid;else if(Object.hasOwn(source,column))row[column]=source[column];else row[column]=null}
     rows.push(row)
    }
   }
   if(rows.length){tables[key]=rows;sections.push({key,count:rows.length,checksum:checksum(rows)})}
  }
  if(!sections.length)throw new ValidationError('Backup Google Drive tidak memiliki bagian data yang didukung')
  return{manifest:{format:'jurnalku-tenant-backup',schema_version:1,created_at:legacy.exported_at||new Date().toISOString(),tenant,users_excluded:true,excluded:EXCLUDED,legacy_source:{tenant_id:legacy.tenant_id||null,slug:legacy.slug||null},sections},tables}
 }
 function parseArtifact(tid,buffer){
  if(!Buffer.isBuffer(buffer)||!buffer.length)throw new ValidationError('File backup wajib')
  let raw=buffer
  if(buffer[0]===0x1f&&buffer[1]===0x8b){try{raw=zlib.gunzipSync(buffer)}catch{throw new ValidationError('File GZIP rusak atau tidak valid')}}
