...
waQueue.setupWA(db)
notificationMonitor.setupMonitoring(db)
...
db.exec(`
  ...
  -- Jadwal ujian: tabel TERPISAH dari jadwal reguler (tabel jadwal) secara sengaja.
  -- Slot ujian sering tumpang-tindih jam dengan jadwal reguler (ujian MENGGANTIKAN
  -- jadwal reguler pada tanggal ujian), sehingga tidak boleh divalidasi anti-bentrok
  -- terhadap tabel jadwal (lihat detectJadwalConflicts / POST /api/jadwal).
  ...
  CREATE INDEX IF NOT EXISTS idx_jadwal_ujian_tenant ON jadwal_ujian(tenant_id);
  CREATE INDEX IF NOT EXISTS idx_jadwal_ujian_rombel_hari ON jadwal_ujian(tenant_id, rombel_id, hari);
  CREATE INDEX IF NOT EXISTS idx_jadwal_ujian_gtk_hari ON jadwal_ujian(tenant_id, gtk_id, hari);
  ...
  CREATE TABLE IF NOT EXISTS sesi_kelas_guru (
    id TEXT PRIMARY KEY,
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS idx_sesi_kelas_jadwal_tanggal ON sesi_kelas_guru(tenant_id, jadwal_id, tanggal);
  CREATE INDEX IF NOT EXISTS idx_sesi_kelas_tenant_tanggal ON sesi_kelas_guru(tenant_id, tanggal, status);
  ...
  CREATE TABLE IF NOT EXISTS absensi_mapel (
    ...
    created_at TEXT DEFAULT (datetime('now')),
    updated_at TEXT,
    UNIQUE(tenant_id,siswa_id,jadwal_id,tanggal),
    FOREIGN KEY (siswa_id) REFERENCES siswa(id),
    FOREIGN KEY (rombel_id) REFERENCES rombel(id),
  ...
  CREATE INDEX IF NOT EXISTS idx_absensi_mapel_tenant_siswa ON absensi_mapel(tenant_id,siswa_id,tanggal);
  CREATE INDEX IF NOT EXISTS idx_absensi_mapel_tenant_jadwal ON absensi_mapel(tenant_id,jadwal_id,tanggal);
...
try {
  ...
  if (!userColumns.some(column => column.name === 'can_teach')) {
...
function userHasCapability(user, capability) {
  if (capability !== 'teacher' || !user) return false
  if (['guru', 'wali_kelas'].includes(user.role)) return true
  if (user.role !== 'kepala' || !user.tenant_id) return false
  const stored = db.prepare('SELECT can_teach FROM users WHERE id=? AND tenant_id=?').get(user.id, user.tenant_id)
  return !!stored?.can_teach && !!resolveGtkForUser(user.id, user.tenant_id)
}
function isTeacherContext(req) {
  return ['guru', 'wali_kelas'].includes(req.user?.role)
    || (String(req.headers['x-access-mode'] || '') === 'teacher' && userHasCapability(req.user, 'teacher'))
}
function requireCapability(capability) {
  return (req, res, next) => authMiddleware(req, res, () => {
    if (!userHasCapability(req.user, capability)) return res.status(403).json({ error: 'Akses ditolak: capability tidak tersedia' })
    next()
  })
}
...
app.get('/api/auth/me', authMiddleware, (req, res) => {
  const user = db.prepare('SELECT id, nama, email, role, nip, nis, avatar, gtk_id, can_teach FROM users WHERE id = ? AND tenant_id = ?').get(req.user.id, req.tenantId)
...
app.get('/api/users', ADMIN, (req, res) => {
  ...
  res.json(rows.map(row => ({ ...row, can_teach: !!row.can_teach, student_ids: links.all(req.tenantId, row.id).map(x => x.student_id) })))
})
...
app.post('/api/users', ADMIN, (req, res) => {
  const { nama, email, password, role, student_ids = [], can_teach = false } = req.body
  if (!nama || !nama.trim()) return res.status(400).json({ error: 'Nama wajib diisi' })
  ...
  const id = uuidv4()
  db.transaction(() => {
    db.prepare('INSERT INTO users (id, nama, email, password, role, tenant_id, can_teach) VALUES (?,?,?,?,?,?,?)')
      .run(id, nama.trim(), email.trim(), bcrypt.hashSync(password, 10), role, req.tenantId, role === 'kepala' && can_teach ? 1 : 0)
  ...
  res.json({ id, nama, email, role, can_teach: role === 'kepala' && !!can_teach })
})

app.put('/api/users/:id', ADMIN, (req, res) => {
  const { nama, email, role, password, student_ids, can_teach } = req.body
  ...
  db.transaction(() => {
    db.prepare('UPDATE users SET nama = COALESCE(?, nama), email = COALESCE(?, email), role = COALESCE(?, role), can_teach = ? WHERE id = ? AND tenant_id = ?')
      .run(nama || null, email || null, role || null, finalRole === 'kepala' && (can_teach === undefined ? target.can_teach : can_teach) ? 1 : 0, req.params.id, req.tenantId)
...
app.post('/api/settings/reset-data', ADMIN, (req, res) => {
  ...
  const tables = [
    ...
    'absensi_siswa', 'absensi_guru', 'jurnal_mengajar', 'sesi_kelas_guru',
...
const BULK_DELETE_MAP = {
  ...
  gtk: { label: 'GTK', main: 'gtk', cascade: ['sesi_kelas_guru', 'jadwal', 'pengajar', 'modul_ajar', 'absensi_guru', 'jurnal_mengajar'] },
  ...
  jadwal: { label: 'Jadwal', main: 'jadwal', cascade: ['sesi_kelas_guru'] },
...
function teacherScheduleForDay(gtkId, tenantId, day, date, examTemplateId = null) {
  if (examTemplateId) {
    const exam = db.prepare(`SELECT j.*, m.nama mapel_nama, r.nama rombel_nama,
      ...
      WHERE j.gtk_id=? AND lower(j.hari)=? AND j.tenant_id=? AND j.template_id=?`)
      .all(gtkId, date, gtkId, day, tenantId, examTemplateId)
      .map(row => ({ ...row, jenis_kegiatan: 'mapel' }))
    return exam.sort((a, b) => String(a.jam_mulai || '').localeCompare(String(b.jam_mulai || '')))
  }
...
function jadwalUntukRombelHari(rombelId, tenantId, hari, examTemplateId) {
  if (examTemplateId) {
    return db.prepare(`SELECT j.*, m.nama as mapel_nama, g.nama as guru_nama, g.nama as gtk_nama,
      ...
      WHERE j.tenant_id=? AND j.rombel_id=? AND lower(trim(coalesce(j.hari,'')))=? AND j.template_id=?
      ORDER BY j.jam_mulai`).all(tenantId, rombelId, hari, examTemplateId)
  }
  return db.prepare(`SELECT j.*, m.nama as mapel_nama, g.nama as guru_nama, g.nama as gtk_nama,
    CASE WHEN g.id IS NULL THEN 0 ELSE 1 END as guru_valid
    FROM jadwal j LEFT JOIN mapel m ON j.mapel_id=m.id AND m.tenant_id=j.tenant_id
    LEFT JOIN gtk g ON j.gtk_id=g.id AND g.tenant_id=j.tenant_id
    WHERE j.tenant_id=? AND j.rombel_id=? AND lower(trim(coalesce(j.hari,'')))=?
    ORDER BY j.jam_mulai`).all(tenantId, rombelId, hari)
}
...
app.post('/api/guru/sesi-kelas/masuk', TEACHER, (req, res) => {
  const gtk = resolveGtkForUser(req.user.id, req.tenantId)
  ...
  if (active && active.jadwal_id !== jadwal.id) return res.status(409).json({ error: `Selesaikan dahulu kelas ${active.rombel_nama} - ${active.mapel_nama}` })
  if (active) return res.json(active)
  const now = timeJakarta()
  const nowMinutes = clockToMinutes(now)
  const startMinutes = clockToMinutes(jadwal.jam_mulai)
  const endMinutes = clockToMinutes(jadwal.jam_selesai)
  if (nowMinutes < startMinutes - 30 || nowMinutes > endMinutes + 60) {
    return res.status(400).json({ error: `Masuk kelas hanya tersedia 30 menit sebelum sampai 60 menit sesudah jadwal ${jadwal.jam_mulai}-${jadwal.jam_selesai}` })
  }
...
app.post('/api/guru/sesi-kelas/selesai', TEACHER, (req, res) => {
  const gtk = resolveGtkForUser(req.user.id, req.tenantId)
  if (!gtk) return res.status(400).json({ error: 'Akun guru belum terhubung GTK' })
  const today = todayJakarta()
  const sesiId = String(req.body?.sesi_id || '').trim()
  const active = currentTeacherClassSession(req.tenantId, gtk.id, today)
  if (!active) return res.status(404).json({ error: 'Tidak ada sesi kelas aktif' })
  if (sesiId && active.id !== sesiId) return res.status(409).json({ error: 'Sesi kelas tidak sesuai dengan sesi aktif' })
  db.prepare("UPDATE sesi_kelas_guru SET status='selesai',waktu_selesai=?,updated_at=datetime('now') WHERE id=? AND tenant_id=? AND guru_id=?")
    .run(timeJakarta(), active.id, req.tenantId, gtk.id)
  const updated = db.prepare('SELECT * FROM sesi_kelas_guru WHERE id=? AND tenant_id=? AND guru_id=?').get(active.id, req.tenantId, gtk.id)
  if (!updated || updated.status !== 'selesai') return res.status(500).json({ error: 'Sesi kelas gagal disimpan' })
  try { notificationMonitor.logActivity(db, { tenantId: req.tenantId, eventType: 'class_session_finished', actorId: gtk.id, entityId: updated.id, metadata: { jadwal_id: updated.jadwal_id } }) } catch {}
  res.json(updated)
})
...
app.get('/api/admin/sesi-kelas/hari-ini', DASHBOARD_ROLES, (req, res) => {
  ...
  res.json({ tanggal, sessions, summary: {
    ...
    selesai: sessions.filter(row => row.status === 'selesai').length,
    terlambat: sessions.filter(row => Number(row.menit_terlambat) > 0).length,
    total: sessions.length
  } })
})

app.get('/api/admin/monitoring', DASHBOARD_ROLES, (req, res) => {
  const tanggal = /^\d{4}-\d{2}-\d{2}$/.test(String(req.query.tanggal || '')) ? String(req.query.tanggal) : todayJakarta()
...
// Seluruh identitas jadwal, guru, siswa, dan catatan dibatasi ke tenant aktif.
app.get('/api/absensi-mapel', STAFF, (req, res) => {
  const tanggal = String(req.query.tanggal || todayJakarta())
  ...
  if (isTeacherContext(req)) {
    if (!jadwalId) return res.status(400).json({ error: 'jadwal_id wajib untuk guru' })
    const gtk = resolveGtkForUser(req.user.id, req.tenantId)
    const jadwal = db.prepare('SELECT mapel_id,rombel_id FROM jadwal WHERE id=? AND tenant_id=?').get(jadwalId, req.tenantId)
    if (!gtk || !jadwal || !teacherCanTeachPair(gtk.id, req.tenantId, jadwal.mapel_id, jadwal.rombel_id, tanggal)) return res.status(403).json({ error: 'Jadwal bukan yang Anda ampu' })
  }
  let sql = `SELECT am.*,s.nama AS siswa_nama,s.nis,m.nama AS mapel_nama,r.nama AS rombel_nama
    FROM absensi_mapel am
    JOIN siswa s ON s.id=am.siswa_id AND s.tenant_id=am.tenant_id
    LEFT JOIN mapel m ON m.id=am.mapel_id AND m.tenant_id=am.tenant_id
    LEFT JOIN rombel r ON r.id=am.rombel_id AND r.tenant_id=am.tenant_id
    WHERE am.tenant_id=? AND am.tanggal=?`
  const params = [req.tenantId, tanggal]
  if (jadwalId) { sql += ' AND am.jadwal_id=?'; params.push(jadwalId) }
  if (rombelId) { sql += ' AND am.rombel_id=?'; params.push(rombelId) }
  sql += ' ORDER BY s.nama'
  res.json(db.prepare(sql).all(...params))
})

app.post('/api/absensi-mapel/bulk', STAFF, (req, res) => {
  const { jadwal_id, tanggal, data } = req.body || {}
  const date = String(tanggal || '').trim()
  if (!jadwal_id || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !Array.isArray(data)) return res.status(400).json({ error: 'jadwal_id, tanggal, dan data wajib valid' })
  const gtk = resolveGtkForUser(req.user.id, req.tenantId)
  const jadwal = db.prepare('SELECT id,mapel_id,rombel_id,gtk_id FROM jadwal WHERE id=? AND tenant_id=?').get(jadwal_id, req.tenantId)
  if (!jadwal) return res.status(404).json({ error: 'Jadwal tidak ditemukan' })
  if (isTeacherContext(req) && (!gtk || !teacherCanTeachPair(gtk.id, req.tenantId, jadwal.mapel_id, jadwal.rombel_id, date))) return res.status(403).json({ error: 'Jadwal bukan yang Anda ampu' })
  const statuses = new Set(['hadir','sakit','izin','alpha','alpa'])
  const studentExists = db.prepare("SELECT 1 FROM siswa WHERE id=? AND rombel_id=? AND tenant_id=? AND COALESCE(status,'aktif')='aktif'")
  if (data.some(row => !row?.siswa_id || !statuses.has(String(row.status || '').toLowerCase()) || !studentExists.get(row.siswa_id, jadwal.rombel_id, req.tenantId))) return res.status(400).json({ error: 'Siswa atau status absensi tidak valid' })
  const upsert = db.prepare(`INSERT INTO absensi_mapel (id,siswa_id,rombel_id,jadwal_id,mapel_id,guru_id,tanggal,status,keterangan,tenant_id,updated_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,datetime('now'))
    ON CONFLICT(tenant_id,siswa_id,jadwal_id,tanggal) DO UPDATE SET status=excluded.status,keterangan=excluded.keterangan,updated_at=datetime('now')`)
  const tx = db.transaction(rows => { for (const row of rows) upsert.run(uuidv4(), row.siswa_id, jadwal.rombel_id, jadwal.id, jadwal.mapel_id, gtk?.id || jadwal.gtk_id, date, String(row.status).toLowerCase() === 'alpa' ? 'alpha' : String(row.status).toLowerCase(), row.keterangan || '', req.tenantId) })
  tx(data)
  res.json({ count: data.length, tanggal: date, jadwal_id: jadwal.id })
})
...
app.post('/api/guru/tugas', TEACHER, (req, res) => {
  ...
  try { notificationMonitor.logActivity(db, { tenantId: req.tenantId, eventType: 'assignment_created', actorId: gtk.id, entityId: id, metadata: { rombel_id, deadline: deadline || null } }) } catch {}
  res.json({ id })
})
...
app.get('/api/siswa/dashboard', authMiddleware, enforceTenantAccess, (req, res) => {
  ...
  if (siswa.rombel_id) {
    ...
    jadwal = jadwalUntukRombelHari(siswa.rombel_id, req.tenantId, today, examTemplateId)
  }
  ...
  const attendance = studentAttendance(db, req.tenantId, siswa.id, 100)
  const absensi_detail = attendance.qr_masuk_pulang_detail.slice(0, 20)
...
app.get('/api/jadwal/hari-ini', DASHBOARD_ROLES, (req, res) => {
  ...
  const rows = jadwalUntukTenantHari(req.tenantId, hari, examTemplateId)
...
app.get('/api/jadwal/tanggal', DASHBOARD_ROLES, (req, res) => {
  ...
  const examTemplateId = examModeForDate(req.tenantId, tanggal)
  const rows = tenantIsHoliday(req.tenantId, tanggal) ? [] : jadwalUntukTenantHari(req.tenantId, hari, examTemplateId)
...
// ==================== JADWAL UJIAN (tabel terpisah, tanpa validasi bentrok thd jadwal reguler) ====================
// Slot ujian sengaja tumpang-tindih jam dengan jadwal reguler (menggantikannya saat mode ujian aktif),
// jadi anti-bentrok GURU/RUANGAN di sini hanya dicek SESAMA jadwal_ujian, TIDAK terhadap tabel jadwal.
app.get('/api/jadwal-ujian', authMiddleware, (req, res) => {
  const { template_id, rombel_id } = req.query
  let sql = `SELECT j.*, m.nama as mapel_nama, m.kode as mapel_kode, r.nama as rombel_nama,
    g.nama as gtk_nama, g.nama as guru_nama, CASE WHEN g.id IS NULL THEN 0 ELSE 1 END as guru_valid
    FROM jadwal_ujian j
    ...
    WHERE j.tenant_id=?`
  const params = [req.tenantId]
  if (template_id) { sql += ' AND j.template_id=?'; params.push(template_id) }
  if (rombel_id) { sql += ' AND j.rombel_id=?'; params.push(rombel_id) }
  sql += ' ORDER BY j.hari, j.jam_mulai'
  res.json(db.prepare(sql).all(...params))
})

app.post('/api/jadwal-ujian', ADMIN, (req, res) => {
  const { template_id, mapel_id, rombel_id, gtk_id, hari, jam_mulai, jam_selesai, ruangan } = req.body
  if (!template_id) return res.status(400).json({ error: 'Template ujian wajib dipilih.' })
  if (!rombel_id) return res.status(400).json({ error: 'Rombel wajib dipilih.' })
  if (!hari || !jam_mulai || !jam_selesai) return res.status(400).json({ error: 'Hari, jam mulai, dan jam selesai wajib diisi.' })
  const tpl = db.prepare("SELECT id FROM template_jadwal WHERE id=? AND tenant_id=? AND jenis='ujian'").get(template_id, req.tenantId)
  if (!tpl) return res.status(400).json({ error: 'Template ujian tidak ditemukan atau bukan jenis ujian.' })
  // Anti-bentrok HANYA sesama jadwal_ujian dgn template sama (guru & ruangan), tidak terhadap jadwal reguler.
  const overlap = '((j.jam_mulai < ? AND j.jam_selesai > ?) OR (j.jam_mulai < ? AND j.jam_selesai > ?) OR (j.jam_mulai >= ? AND j.jam_selesai <= ?))'
  const ovParams = [jam_selesai, jam_mulai, jam_selesai, jam_mulai, jam_mulai, jam_selesai]
  if (gtk_id) {
    const gConflict = db.prepare(`SELECT j.*, r.nama as rombel_nama FROM jadwal_ujian j LEFT JOIN rombel r ON j.rombel_id=r.id WHERE j.gtk_id=? AND j.hari=? AND j.template_id=? AND j.tenant_id=? AND ${overlap}`)
      .get(gtk_id, hari, template_id, req.tenantId, ...ovParams)
    if (gConflict) return res.status(409).json({ error: `Guru pengawas sudah bertugas di ${gConflict.rombel_nama} jam ${gConflict.jam_mulai}-${gConflict.jam_selesai} (${hari})` })
  }
  const rConflict = db.prepare(`SELECT j.*, m.nama as mapel_nama FROM jadwal_ujian j LEFT JOIN mapel m ON j.mapel_id=m.id WHERE j.rombel_id=? AND j.hari=? AND j.template_id=? AND j.tenant_id=? AND ${overlap}`)
    .get(rombel_id, hari, template_id, req.tenantId, ...ovParams)
  if (rConflict) return res.status(409).json({ error: `Kelas sudah ada jadwal ujian ${rConflict.mapel_nama || ''} jam ${rConflict.jam_mulai}-${rConflict.jam_selesai} (${hari})` })
  const id = uuidv4()
  db.prepare(`INSERT INTO jadwal_ujian (id, template_id, mapel_id, rombel_id, gtk_id, hari, jam_mulai, jam_selesai, ruangan, tenant_id)
    VALUES (?,?,?,?,?,?,?,?,?,?)`).run(id, template_id, mapel_id || null, rombel_id, gtk_id || null, hari, jam_mulai, jam_selesai, ruangan || '', req.tenantId)
  res.json({ id })
})

app.put('/api/jadwal-ujian/:id', ADMIN, (req, res) => {
  const existing = db.prepare('SELECT * FROM jadwal_ujian WHERE id=? AND tenant_id=?').get(req.params.id, req.tenantId)
  if (!existing) return res.status(404).json({ error: 'Jadwal ujian tidak ditemukan.' })
  const { mapel_id, rombel_id, gtk_id, hari, jam_mulai, jam_selesai, ruangan } = req.body
  db.prepare(`UPDATE jadwal_ujian SET mapel_id=?, rombel_id=?, gtk_id=?, hari=?, jam_mulai=?, jam_selesai=?, ruangan=? WHERE id=? AND tenant_id=?`)
    .run(mapel_id ?? existing.mapel_id, rombel_id ?? existing.rombel_id, gtk_id ?? existing.gtk_id, hari ?? existing.hari,
      jam_mulai ?? existing.jam_mulai, jam_selesai ?? existing.jam_selesai, ruangan ?? existing.ruangan, req.params.id, req.tenantId)
  res.json({ success: true })
})

app.delete('/api/jadwal-ujian/:id', ADMIN, (req, res) => {
  db.prepare('DELETE FROM jadwal_ujian WHERE id=? AND tenant_id=?').run(req.params.id, req.tenantId)
  res.json({ success: true })
})

app.delete('/api/jadwal-ujian/template/:templateId', ADMIN, (req, res) => {
  const result = db.prepare('DELETE FROM jadwal_ujian WHERE template_id=? AND tenant_id=?').run(req.params.templateId, req.tenantId)
...
app.get('/api/guru/peminatan', STAFF, (req, res) => {
  if (!isTeacherContext(req)) return res.status(403).json({ error: 'Akses khusus guru' })
  const gtk = resolveGtkForUser(req.user.id, req.tenantId)
  if (!gtk) return res.json([])
...
app.post('/api/tahfidz/pertemuan', STAFF, (req,res)=>{
  ...
  if(isTeacherContext(req)){const gtk=resolveGtkForUser(req.user.id,req.tenantId);if(!gtk||group.pembimbing_id!==gtk.id)return res.status(403).json({error:'Bukan pembimbing kegiatan ini'})}
...
app.get('/api/jurnal/jadwal-hari-ini', authMiddleware, (req, res) => {
  ...
  const rows = examTemplateId
    ? db.prepare(`SELECT j.id as jadwal_id, j.mapel_id, j.rombel_id, j.jam_mulai, j.jam_selesai, j.ruangan,
        ...
        LEFT JOIN rombel r ON j.rombel_id = r.id AND r.tenant_id = j.tenant_id
        WHERE j.gtk_id = ? AND lower(j.hari) = ? AND j.tenant_id = ? AND j.template_id = ?
        ORDER BY j.jam_mulai`).all(gtk.id, hari, req.tenantId, examTemplateId)
    : db.prepare(`SELECT j.id as jadwal_id, j.mapel_id, j.rombel_id, j.jam_mulai, j.jam_selesai, j.ruangan,
        m.nama as mapel_nama, m.kode as mapel_kode, r.nama as rombel_nama
        FROM jadwal j
...
app.post('/api/jurnal', STAFF, signatureUpload.single('signature'), async (req, res, next) => {
  ...
  const teacher = isTeacherContext(req)
  if (teacher) {
    const gtk = resolveGtkForUser(req.user.id, req.tenantId)
...
app.put('/api/jurnal/:id', STAFF, (req, res) => {
  ...
  if (reviewer && ['approved','rejected'].includes(status)) {
    ...
    if (!isTeacherContext(req) || !['draft','submitted'].includes(status)) return res.status(403).json({ error: 'Status jurnal tidak diizinkan' })
    const gtk = resolveGtkForUser(req.user.id, req.tenantId)
    if (!gtk) return res.status(403).json({ error: 'Akun guru belum terhubung ke GTK' })
...
app.delete('/api/jurnal/:id', STAFF, (req, res) => {
  const reviewer = ['admin','super_admin','kepala','operator'].includes(req.user.role)
  if (!reviewer && !isTeacherContext(req)) return res.status(403).json({ error: 'Role tidak diizinkan menghapus jurnal' })
  let result
  if (reviewer) {
...
app.post('/api/rapor/generate', STAFF, (req, res) => {
  ...
  const count = generateRaporForRombel(db, {
    tenantId: req.tenantId,
    rombelId: String(rombel_id),
...
// ==================== LEDGER NILAI ====================
const LEDGER = requireRole('admin', 'super_admin', 'wali_kelas')

function authorizedLedgerRombel(req, rombelId) {
  if (!rombelId) return null
  if (req.user.role === 'wali_kelas') {
    const gtk = resolveGtkForUser(req.user.id, req.tenantId)
    if (!gtk) return null
    return db.prepare(`SELECT id, nama, tahun_ajaran FROM rombel
      WHERE id=? AND tenant_id=? AND wali_kelas_id=?`).get(String(rombelId), req.tenantId, gtk.id) || null
  }
  return db.prepare(`SELECT id, nama, tahun_ajaran FROM rombel
    WHERE id=? AND tenant_id=?`).get(String(rombelId), req.tenantId) || null
}

function ledgerRequest(req, res) {
  const { rombel_id, tahun_ajaran, semester, jenis = 'semua' } = req.query
  if (!rombel_id || !tahun_ajaran || !semester) {
    res.status(400).json({ error: 'rombel_id, tahun_ajaran, semester wajib' })
    return null
  }
  if (!['ganjil', 'genap'].includes(semester) || !['rapor_sts', 'rapor_sas', 'semua'].includes(jenis)) {
    res.status(400).json({ error: 'Semester atau jenis rapor tidak valid' })
    return null
  }
  const rombel = authorizedLedgerRombel(req, rombel_id)
  if (!rombel || rombel.tahun_ajaran !== tahun_ajaran) {
...
app.get('/api/rapor/ledger/rombel', LEDGER, (req, res) => {
  ...
  res.json(getAuthorizedLedgerRombels(db, {
    tenantId: req.tenantId,
    role: req.user.role,
    gtkId: gtk?.id,
  }))
...
app.get('/api/rapor/ledger', LEDGER, (req, res) => {
...
app.get('/api/rapor/ledger/export', LEDGER, async (req, res) => {
  const input = ledgerRequest(req, res)
  if (!input) return
  try {
    const rows = getLedgerRows(db, {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="Ledger-${input.rombel.id}-${input.tahun_ajaran}-${input.semester}.xlsx"`)
