...
const log=pino({level:process.env.LOG_LEVEL||'info'}); const sockets=new Map(); const stopping=new Set()
...
function session(tenantId,status,extra={}){db.prepare(`INSERT INTO wa_sessions(tenant_id,status,qr,last_error,phone,updated_at) VALUES(?,?,?,?,?,datetime('now')) ON CONFLICT(tenant_id) DO UPDATE SET status=excluded.status,qr=excluded.qr,last_error=excluded.last_error,phone=COALESCE(excluded.phone,wa_sessions.phone),updated_at=datetime('now')`).run(tenantId,status,extra.qr||null,extra.error||null,extra.phone||null)}
async function connect(tenantId){
  ...
  const sock=makeWASocket({auth:state,logger:log.child({tenantId}),printQRInTerminal:false}); sockets.set(tenantId,sock); session(tenantId,'connecting')
  sock.ev.on('creds.update',saveCreds)
  sock.ev.on('connection.update',({connection,lastDisconnect,qr})=>{
    if(qr)session(tenantId,'qr',{qr}) // QR stored for admin endpoint; never logged
    if(connection==='open')session(tenantId,'connected',{phone:sock.user?.id?.split(':')[0]})
    if(connection==='close'){
      ...
      else {session(tenantId,'reconnecting',{error:String(lastDisconnect?.error?.message||'connection closed').slice(0,500)}); setTimeout(()=>connect(tenantId).catch(e=>log.error({tenantId,err:e.message},'reconnect')),5000)}
    }
  })
}
async function tick(){
  ...
  for(const c of db.prepare("SELECT tenant_id FROM wa_gateway_config WHERE enabled=1 AND provider='baileys'").all()){
    ...
    try {const r=await sock.sendMessage(`${job.phone}@s.whatsapp.net`,{text:job.message}); db.prepare("UPDATE wa_queue SET status='sent',sent_at=datetime('now'),message_id=?,last_error=NULL WHERE id=? AND tenant_id=?").run(r.key?.id||null,job.id,c.tenant_id)}
    catch(e){db.prepare("UPDATE wa_queue SET status='failed',failed_at=datetime('now'),last_error=?,available_at=datetime('now',printf('+%d minutes',MIN(attempts,5)*2)) WHERE id=? AND tenant_id=?").run(String(e.message).slice(0,500),job.id,c.tenant_id)}
  }
}
