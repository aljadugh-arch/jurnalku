...
export default function PortalSheet({ open, onClose, title, description, children, align = 'bottom' }: PortalSheetProps) {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[100] flex justify-center bg-black/50 p-0 sm:p-4 ${align === 'top' ? 'items-start sm:pt-16' : 'items-end sm:items-center'}`} onMouseDown={e => e.target === e.currentTarget && onClose()}>
      <div ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="portal-sheet-title" className={`flex w-full sm:max-w-2xl max-h-[88dvh] flex-col bg-white dark:bg-gray-900 shadow-2xl outline-none ${align === 'top' ? 'rounded-b-3xl sm:rounded-3xl' : 'rounded-t-3xl sm:rounded-3xl'}`}>
        <div className="flex shrink-0 items-start justify-between gap-4 bg-white dark:bg-gray-900 p-5 pb-4">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto overscroll-contain px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
          {children}
        </div>
      </div>
    </div>, document.body
