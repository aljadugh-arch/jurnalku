...
export default function MobileHeader({
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      <button
        onClick={onBell}
        className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/15 text-white backdrop-blur-sm active:scale-95 transition"
        aria-label="Posting"
      >
        <Bell size={20} />
      </button>

      {/* Theme toggle: dipindah ke sebelah bell notif & foto profil */}
      <button
        onClick={toggleDark}
        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/15 text-white backdrop-blur-sm active:scale-95 transition"
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-1.5 rounded-full bg-white/15 p-0.5 pr-1.5 backdrop-blur-sm active:scale-95 transition"
          aria-label="Menu akun"
        >
          <Avatar src={user?.avatar} name={user?.nama} size={34} className="!border-2 !border-white/40 !shadow-none" />
