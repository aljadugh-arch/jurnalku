...
export default function GuruNilaiSumatifPage() {
  ...
  const loadSiswaDanNilai = async () => {
    ...
    try {
      ...
      setSiswaList(siswa)
      const { data: rapor } = await api.get('/rapor', { params: { tahun_ajaran: tahunAjaran, semester, jenis: 'sumatif' } })
      const seed: Record<string, NilaiEntry> = {}
      for (const s of siswa) {
        const existing = rapor.find((r: any) => r.siswa_id === s.id && r.mapel_id === selectedMapel)
        seed[s.id] = { sts: existing?.nilai_sts || '', sas: existing?.nilai_sas || '' }
      }
      setNilai(seed)
    } catch (e) {
      console.error(e)
  ...
  const handleSave = async () => {
    ...
    try {
      const items = siswaList.map(s => ({
        ...
        nilai_sts: nilai[s.id]?.sts === '' ? 0 : nilai[s.id]?.sts,
        nilai_sas: nilai[s.id]?.sas === '' ? 0 : nilai[s.id]?.sas,
      }))
      const { data } = await api.post('/rapor/nilai-sumatif', { tahun_ajaran: tahunAjaran, semester, items })
      setMsg(`✓ ${data.message}`)
    } catch (e: any) {
      setMsg(`✗ ${e.response?.data?.error || 'Gagal menyimpan nilai sumatif'}`)
    } finally {
      setSaving(false)
    }
  }
                ...
                {siswaList.map((s, idx) => (
                  <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                    ...
                    <td className="px-4 py-3">
                      <input type="number" min="0" max="100" value={nilai[s.id]?.sts ?? ''}
                    ...
                    <td className="px-4 py-3">
                      <input type="number" min="0" max="100" value={nilai[s.id]?.sas ?? ''}
