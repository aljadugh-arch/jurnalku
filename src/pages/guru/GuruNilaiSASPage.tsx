...
export default function GuruNilaiSASPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-gray-800 dark:text-white">Nilai Asesmen SAS</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
          Input nilai Sumatif Akhir Semester (SAS) — hasil asesmen ujian akhir semester per mapel yang Anda ampu
        </p>
      </div>
              ...
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-40">
                    Nilai SAS<br/><span className="font-normal normal-case">(Asesmen Ujian, 0–100)</span>
                  </th>
                </tr>
              </thead>
      ...
      {!loading && (!selectedMapel || !selectedRombel) && (
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border p-12 text-center text-gray-400">
          ...
          <p>Pilih mata pelajaran dan kelas untuk mulai input nilai SAS</p>
        </div>
      )}

      <div className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1.5">
        <ScrollText size={14} />
        Nilai SAS digunakan saat admin men-generate Rapor SAS: <strong>Nilai Rapor SAS = Nilai Harian × 40% + Asesmen STS × 20% + Asesmen SAS × 40%</strong>
      </div>
    </div>
  )
}
