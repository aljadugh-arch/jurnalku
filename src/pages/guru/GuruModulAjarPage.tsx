...
type HistoryItem = { id: string; type: DocumentType; title: string; subject: string; grade: string; topic: string; created_at: string }
...
export default function GuruModulAjarPage() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      loadHistory()
      toast.success(`${selected.label} berhasil dibuat`)
    } catch (error: any) {
  ...
  const openHistory = async (id: string) => {
    try {
      ...
      setForm(prev => ({ ...prev, ...data.metadata, type: data.type }))
      setResult(data.content)
      setDocumentId(data.id)
  ...
  return <div className="space-y-6">
    <div>
      <h1 className="text-2xl font-bold text-gray-800 dark:text-slate-100">Generator Administrasi Guru AI</h1>
      <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Soal, perangkat ajar, dan perencanaan pembelajaran dalam satu halaman.</p>
    </div>
    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_310px]">
      <div className="space-y-5">
        ...
        <button onClick={generate} disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-primary to-blue-700 px-6 py-3.5 text-sm font-semibold text-white shadow-lg disabled:opacity-50">{loading ? <><Loader2 size={18} className="animate-spin" /> AI sedang menyusun...</> : <><Sparkles size={18} /> Generate {selected.label}</>}</button>
      ...
      <aside className="h-fit rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 xl:sticky xl:top-4">
        ...
        <div className="max-h-[720px] space-y-2 overflow-y-auto pr-1">{history.length === 0 && <p className="text-sm text-gray-400">Belum ada dokumen.</p>}{history.map(item => <button key={item.id} onClick={() => openHistory(item.id)} className="w-full rounded-xl border border-gray-100 p-3 text-left hover:border-primary/40 hover:bg-gray-50 dark:border-slate-800 dark:hover:bg-slate-800"><span className="text-[10px] font-bold text-primary">{item.type}</span><strong className="mt-1 block text-sm text-gray-800 dark:text-slate-100">{item.subject} — {item.grade}</strong><span className="mt-1 line-clamp-2 block text-xs text-gray-500">{item.topic}</span></button>)}</div>
      </aside>
    </div>
  </div>
}
