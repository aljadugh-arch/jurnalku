...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Siswa', value: stats.total_siswa, icon: <GraduationCap size={20} />, gradient: 'from-blue-500 to-indigo-600', sub: `${stats.siswa_aktif} aktif` },
    { label: 'Total GTK', value: stats.total_gtk, icon: <Users size={20} />, gradient: 'from-green-500 to-emerald-600', sub: `${stats.gtk_aktif} aktif` },
    { label: 'Rombel', value: stats.total_rombel, icon: <Layers size={20} />, gradient: 'from-purple-500 to-violet-600', sub: '' },
    { label: 'Mata Pelajaran', value: stats.total_mapel, icon: <BookOpen size={20} />, gradient: 'from-orange-500 to-amber-600', sub: '' },
    { label: 'Jurnal Hari Ini', value: stats.jurnal_hari_ini, icon: <ClipboardList size={20} />, gradient: 'from-cyan-500 to-sky-600', sub: '' },
    { label: 'Tagihan Belum Bayar', value: stats.tagihan?.belum_bayar || 0, icon: <DollarSign size={20} />, gradient: 'from-rose-500 to-red-600', sub: `${stats.tagihan?.lunas || 0} lunas` },
  ]
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {statCards.map((stat) => (
          <StatCard key={stat.label} label={stat.label} value={stat.value} icon={stat.icon} gradient={stat.gradient} sub={stat.sub} />
        ))}
      </div>
