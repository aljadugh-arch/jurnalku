...
import { Plus, Trash2, X, Users, Download, Upload } from 'lucide-react'
...
export default function RombelPage() {
  ...
  const handleSave = async () => {
    const nama = composeNama(jenjang, form.tingkat, paralel)
    if (!nama || !form.tingkat) { toast.error('Tingkat wajib'); return }
    // cek nama ganda (auto, client-side)
    if (data.some(r => r.nama.toLowerCase() === nama.toLowerCase() && r.tahun_ajaran === form.tahun_ajaran)) {
      toast.error(`Rombel "${nama}" sudah ada di tahun ${form.tahun_ajaran}`); return
    }
    try {
      await api.post('/rombel', { ...form, nama })
      toast.success('Rombel berhasil ditambahkan')
      setShowModal(false); setForm({ nama: '', tingkat: tOpts[0] || 'I', tahun_ajaran: '2024/2025', wali_kelas_id: '', kapasitas: 36 }); setParalel(paralelMode === 'numerik' ? '1' : 'A'); fetchData()
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="text-xs bg-gray-50 rounded-lg p-2 text-gray-600">
                Nama rombel: <span className="font-semibold text-gray-800">{composeNama(jenjang, form.tingkat, jenjang === 'RA' ? '' : paralel)}</span>
              </div>
